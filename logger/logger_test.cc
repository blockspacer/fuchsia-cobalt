...
TEST_F(LoggerTest, LogEvent) {
  ...
  ASSERT_EQ(kInvalidArguments,
            logger_->LogEvent(testing::all_report_types::kErrorOccurredMetricId, 101));
...
TEST_F(LoggerTest, LogEventCount) {
  ...
  // Attempt to use an event code larger than max_event_code.
  ASSERT_EQ(kInvalidArguments,
            logger_->LogEventCount(testing::all_report_types::kReadCacheHitsMetricId, 112,
                                   "component2", 1, 303));
  ...
  ASSERT_EQ(kOK, logger_->LogEventCount(testing::all_report_types::kReadCacheHitsMetricId, 0, "", 1,
                                        303));
...
TEST_F(LoggerTest, LogElapsedTime) {
  ...
  ASSERT_EQ(kInvalidArguments,
            logger_->LogElapsedTime(testing::all_report_types::kModuleLoadTimeMetricId, 44,
                                    "component4", 4004));
...
TEST_F(LoggerTest, LogFrameRate) {
  ...
  ASSERT_EQ(kInvalidArguments,
            logger_->LogFrameRate(testing::all_report_types::kLoginModuleFrameRateMetricId, 0,
                                  "component5", 5.123));
...
TEST_F(LoggerTest, LogMemoryUsageMultiDimension) {
  ...
  // Use a metric ID for the wrong type of metric. Expect kInvalidArguments.
  ASSERT_EQ(kInvalidArguments,
            logger_->LogMemoryUsage(testing::all_report_types::kLoginModuleFrameRateMetricId,
...
TEST_F(LoggerTest, CheckNumAggregatedObsImmediateAndAggregatedEvents) {
  ...
  ASSERT_EQ(kOK, logger_->LogEvent(testing::all_report_types::kEventsOccurredMetricId, 0));
  ASSERT_EQ(kOK, logger_->LogEvent(testing::all_report_types::kEventsOccurredMetricId, 0));
  ASSERT_EQ(kOK, logger_->LogEvent(testing::all_report_types::kEventsOccurredMetricId, 2));
...
TEST_F(UniqueActivesLoggerTest, CheckUniqueActivesObsValuesSingleDay) {
  ...
  // Log 2 occurrences of event code 0 for the DeviceBoots metric, which has 1
  // locally aggregated report and no immediate reports.
  ASSERT_EQ(kOK, logger_->LogEvent(testing::unique_actives_noise_free::kDeviceBootsMetricId, 0));
  ASSERT_EQ(kOK, logger_->LogEvent(testing::unique_actives_noise_free::kDeviceBootsMetricId, 0));
  ...
  ASSERT_EQ(kOK, logger_->LogEvent(testing::unique_actives_noise_free::kFeaturesActiveMetricId, 0));
  ASSERT_EQ(kOK, logger_->LogEvent(testing::unique_actives_noise_free::kFeaturesActiveMetricId, 1));
  // Generate locally aggregated observations for the current day index.
  ASSERT_EQ(kOK, GenerateAggregatedObservations(current_day_index));
  ...
  for (uint32_t i = 0; i < 10; i++) {
    if (i < 10) {
      for (uint32_t event_code = 1; event_code < 5; event_code++) {
        if (i % (3 * event_code) == 0) {
          ASSERT_EQ(kOK,
                    logger_->LogEvent(testing::unique_actives_noise_free::kEventsOccurredMetricId,
                                      event_code));
        }
      }
    }
...
TEST_F(PerDeviceNumericLoggerTest, CheckPerDeviceNumericObsValuesSingleDay) {
  ...
  EXPECT_EQ(kOK,
            logger_->LogEventCount(testing::per_device_numeric_stats::kConnectionFailuresMetricId,
                                   0u, "component_A", 0, 5));
  EXPECT_EQ(kOK,
            logger_->LogEventCount(testing::per_device_numeric_stats::kConnectionFailuresMetricId,
                                   0u, "component_B", 0, 5));
  ...
  EXPECT_EQ(kOK,
            logger_->LogEventCount(testing::per_device_numeric_stats::kConnectionFailuresMetricId,
                                   1u, "component_A", 0, 5));
  EXPECT_EQ(kOK, logger_->LogEventCount(testing::per_device_numeric_stats::kSettingsChangedMetricId,
                                        0u, "component_C", 0, 5));
  EXPECT_EQ(kOK, logger_->LogEventCount(testing::per_device_numeric_stats::kSettingsChangedMetricId,
                                        0u, "component_C", 0, 5));
...
// report, on a single day.
TEST_F(PerDeviceNumericLoggerTest, CheckPerDeviceNumericObsValuesFrameRateSingleDay) {
  const auto day_index = CurrentDayIndex(MetricDefinition::UTC);
  // Log several events on |day_index|.
  EXPECT_EQ(kOK,
            logger_->LogFrameRate(testing::per_device_numeric_stats::kLoginModuleFrameRateMetricId,
                                  0u, "component_A", 5.0));
  EXPECT_EQ(kOK,
            logger_->LogFrameRate(testing::per_device_numeric_stats::kLoginModuleFrameRateMetricId,
                                  0u, "component_B", 4.2));
...
// report, on a single day.
TEST_F(PerDeviceNumericLoggerTest, CheckPerDeviceNumericObsValuesMemoryUsageSingleDay) {
  const auto day_index = CurrentDayIndex(MetricDefinition::UTC);
  // Log several events on |day_index|.
  EXPECT_EQ(kOK,
            logger_->LogMemoryUsage(testing::per_device_numeric_stats::kLedgerMemoryUsageMetricId,
  ...
  for (uint32_t offset = 0; offset < num_days; offset++) {
    ...
    for (uint32_t event_code = 1; event_code < 3; event_code++) {
      if (offset > 0 && offset % event_code == 0) {
        EXPECT_EQ(
            kOK, logger_->LogEventCount(testing::per_device_numeric_stats::kSettingsChangedMetricId,
                                        event_code, "A", 0, 3));
      }
...
TEST_F(PerDeviceNumericLoggerTest, ElapsedTimeCheckObservationValues) {
  ...
  for (uint32_t offset = 0; offset < num_days; offset++) {
    ...
    for (uint32_t event_code = 1; event_code < 3; event_code++) {
      if (offset > 0 && offset % event_code == 0) {
        EXPECT_EQ(kOK,
                  logger_->LogElapsedTime(testing::per_device_numeric_stats::kStreamingTimeMetricId,
                                          event_code, "A", 3));
      }
      if (offset > 0 && offset % (2 * event_code) == 0) {
        EXPECT_EQ(kOK,
