...
TEST(SymmetricCipherTest, TestManyStrings) {
  ...
  doSymmetricCipherTest(&cipher, (const byte*)all_lines.data(), all_lines.size());
...
void doHybridCipherTest(HybridCipher* hybrid_cipher, const byte* plain_text, int ptext_len,
  ...
  cipher_text.data()[HybridCipher::PUBLIC_KEY_SIZE] ^= 0x1;  // flip salt bit back
...
TEST(HybridCipherTest, Test) {
  ...
  for (int times = 0; times < 5; ++times) {
    ...
    // Test once with each line separately.
    for (int i = 0; i < kNumLines; i++) {
      doHybridCipherTest(&hybrid_cipher, (const byte*)kLines[i], strlen(kLines[i]), public_key,
                         private_key);
    }
    ...
    doHybridCipherTest(&hybrid_cipher, (const byte*)all_lines.data(), all_lines.size(), public_key,
                       private_key);
